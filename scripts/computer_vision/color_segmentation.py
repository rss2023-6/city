...
def transform_image(img, light=(10, 110, 120), dark=(45, 255, 255)):
    #convert to hsv
...
def convert_line_to_world(line):
  x1, y1, x2, y2 = line[0]
  return [(x1 * 1.0 - CENTER * 1.0) / (SCALE * 1.0), (y1*1.0) / (SCALE*1.0), (x2 * 1.0 - CENTER * 1.0) / (SCALE * 1.0), (y2*1.0) / (SCALE*1.0)]


def cd_color_segmentation(img, template, second_try=False):
    """
    ...
    if(not second_try):
      ...
      linesP = cv2.HoughLinesP(img[:, :, 0], rho=1, theta=np.pi/180, threshold=60, minLineLength=40, maxLineGap=16)
    ...
    plt.plot([lines[-1][0][0], lines[-1][0][2]], [lines[-1][0][1], lines[-1][0][3]])
